...
Сообщение имеет тип `map` и выглядит следуюшим образом:
...
В MoniQue всего 4 типа сообщений: 
  ...
  3. `error` - сообщение с ошибкой. Может быть как ответом на сообщение с типом `config` при неудачном выполнении задачи, а может самостоятельно порождаться каким-либо компонентом;
