...
**timestamp = int** - [Unix-время](https://en.wikipedia.org/wiki/Unix_time) **в миллисекундах**. Данный формат времени настоятельно рекомендуется использовать для времени и в разрабатываемых компонентах.
...
До появления сервиса авторизации данное поле может заполняться произвольно (например, "00000000-0000-0000-имя-разрабатываемого-версиса").
В будущем оно будет специфицировано сервисом авторизации.
...
Данные могут быть любыми, но должны быть сформированы по правилам:
  ...
  3. сообщение в бинарный вид переводится так:
     * если в поле `encoding` указан `MessagePack`, то используется стандартное представление `MessagePack` в бинарном виде;
     * если в поле `endoding` указан `JSON`, то содержимое сначала переводится в строку, а затем в ByteString / byte array.
...
> Пример: для сообщения с соответствующими полями `config`, `ss_ig-annotate`, `YpQzagb9uyRarLvpXXaU9AXChTmkXqXC7pNlBnqf`, `` (нет родительского сообщения) и `00000000-0000-0000-0000-ss_ig-annotate` заголовок
> сообщения будет иметь вид `config:ss_ig-anotate:YpQzagb9uyRarLvpXXaU9AXChTmkXqXC7pNlBnqf::00000000-0000-0000-0000-ss_ig-annotate`.
